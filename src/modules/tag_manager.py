...
class TagManager:
    ...
    def __init__(self, config: object | str | Path | None = None) -> None:
        """
    ...
    def save_tags(self) -> None:
        ...
        with open(self.tags_file, "w", encoding="utf-8") as f:
            json.dump(
                {"tags": sorted(list(self.known_tags))}, f, indent=2, ensure_ascii=False
            )
    ...
    def add_tags(self, new_tags: List[str]) -> int:
        ...
        normalized = {
            tag.strip().lower().replace(" ", "_") for tag in new_tags if tag.strip()
        }
